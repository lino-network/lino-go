...
func (query *Query) GetInfraProvider(ctx context.Context, providerName string) (*model.InfraProvider, error) {
	resp, err := query.transport.Query(ctx, InfraKVStoreKey, infra.QueryInfraProvider, []string{providerName})
	if err != nil {
		linoe, ok := err.(errors.Error)
		if ok && linoe.BlockChainCode() == uint32(linotypes.CodeInfraProviderNotFound) {
...
func (query *Query) GetInfraProviders(ctx context.Context) (*model.InfraProviderList, error) {
	resp, err := query.transport.Query(ctx, InfraKVStoreKey, infra.QueryInfraList, []string{})
	if err != nil {
		return nil, err
	}
